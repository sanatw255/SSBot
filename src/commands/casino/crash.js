...
module.exports = async (client, interaction, args) => {
  ...
  try {
        ...
        .then((msg) => {
          ...
          setInterval(async () => {
            if (index === result + 1) {
              ...
              try {
                const crashData = await Schema.findOne({
                  Guild: interaction.guild.id,
                  User: user.id,
                });
                if (crashData) {
                  crashData.Money -= money;
                  await crashData.save();
                }
              } catch (err) {
                console.error("Error updating crash loss:", err);
              }
          ...
          interaction.channel
            ...
            .then(async (i) => {
              if (i.customId == "crash_stop") {
                ...
                try {
                  const crashWinData = await Schema.findOne({
                    Guild: interaction.guild.id,
                    User: user.id,
                  });
                  if (crashWinData) {
                    crashWinData.Money += parseInt(profit);
                    await crashWinData.save();
                  }
                } catch (err) {
                  console.error("Error updating crash win:", err);
                }
            ...
            .catch(async () => {
              ...
              try {
                const crashTimeoutData = await Schema.findOne({
                  Guild: interaction.guild.id,
                  User: user.id,
                });
                if (crashTimeoutData) {
                  crashTimeoutData.Money -= money;
                  await crashTimeoutData.save();
                }
              } catch (err) {
                console.error("Error updating crash timeout:", err);
              }
    ...
    console.error("Error in crash command:", err);
    client.errNormal(
      { error: "An error occurred while playing crash.", type: "editreply" },
      interaction
    );
  }
};
